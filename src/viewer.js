...
export class Viewer {
    ...
    removeCubes() {
        console.log(this.added_line_names);
        for (var i = 0; i < this.added_line_names.length; i++) {
            this.scene.remove(this.scene.getObjectByName(this.added_line_names[i]));
        }
        this.added_line_names = [];
    }
    ...
    setContent(object, clips) {
        ...
        if (this.options.cameraPosition) {
            ...
            this.defaultCamera.position.x += 4 * Math.sin(theta);
            this.defaultCamera.position.y += 4;
            this.defaultCamera.position.z += 4 * Math.cos(theta);
